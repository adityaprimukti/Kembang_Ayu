...
app.post('/login', (req, res) => {
  ...
  db.query(sql, [username, password], (err, result) => {
    if (err) {
      ...
      res.json({ message: 'Login successful', user: { userId: user.id, username: user.username, email: user.email, phone: user.phone } });
    } else {
...
app.post('/api/reservations', async (req, res) => {
  const { selectedService, selectedBranch, name, phone, email, selectedDay, price, userId } = req.body;
  const status = 'Pending'; // Default status for new reservation

  const sql = 'INSERT INTO tbl_reservations (user_id, service, branch, name, phone, email, day, price, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)';
  db.query(sql, [userId, selectedService, selectedBranch, name, phone, email, selectedDay, price, status], (err, result) => {
    if (err) {
...
app.post('/api/assessments', (req, res) => {
  ...
  const sql = 'INSERT INTO tbl_assessments (name, assessment, day) VALUES (?, ?, ?)';
  db.query(sql, [name, assessment, day], (err, result) => {
    if (err) {
      ...
      res.json({ message: 'Assessment created successfully', assessmentId: result.insertId });
    }
  });
});
...
app.post('/api/contacts', (req, res) => {
  ...
  const sql = 'INSERT INTO tbl_contacts (name, telepon, email, subject, message, branch) VALUES (?, ?, ?, ?, ?, ?)';
  db.query(sql, [name, telepon, email, subject, message, branch], (err, result) => {
    if (err) {
